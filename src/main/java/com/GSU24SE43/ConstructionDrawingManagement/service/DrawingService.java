...
public class DrawingService {
    ...
    public DrawingResponse createDrawing(DrawingRequest request){
        ...
        newDrawing.setSubfolder(subfolder);
    ...
    public List<DrawingResponse> findDrawingsBySubfolderAndStatus(DrawingSearchBySubfolderRequest request, int page, int perPage){
        try {
            ...
            if(!status.isBlank()){
                ...
                drawingResponses = drawingRepository.findBySubfolderAndStatus(subfolder, status)
                        .stream().map(drawingMapper::toDrawingResponse).toList();
            }
            else{
                drawingResponses
                        = drawingRepository.findBySubfolder(subfolder).stream().map(drawingMapper::toDrawingResponse).toList();
            }
